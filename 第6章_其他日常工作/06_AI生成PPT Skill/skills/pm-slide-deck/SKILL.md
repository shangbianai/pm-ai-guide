...
| 支持尺寸 | `2560x1440`（默认，≥3,686,400 像素）、`1024x1024` 等 | `1024x1024`、`1536x1024`、`1024x1536`（默认 `1672x941` 自动转为 `1536x1024`） | 不支持自定义尺寸 | `1024x1024`、`1536x1024`、`1024x1536`（默认 `1672x941` 自动转为 `1536x1024`） |
| 水印 | 关闭 | 不适用 | 不适用 | 不适用 |
...
| `--size <尺寸>` | 图片尺寸：`1672x941`（默认，16:9）。各后端实际使用尺寸不同：豆包自动升档为 `2560x1440`（最低 3,686,400 像素）；OpenAI/Grsai 自动转为 `1536x1024`；Gemini 不支持自定义尺寸 |
...
- 四种后端的图片质量和生成速度各有差异，根据实际需求选择。豆包 Seedream 要求最少 3,686,400 像素，脚本自动将默认 `1672x941` 升档为 `2560x1440`；OpenAI/Grsai 自动转为 `1536x1024`；Gemini 不支持自定义尺寸。
...
- **常用修改项**：
  ...
  - `image_size: 1672x941` —— 默认图片尺寸（`1672x941`）。脚本按后端自动适配：豆包升档为 `2560x1440`，OpenAI/Grsai 转为 `1536x1024`。
