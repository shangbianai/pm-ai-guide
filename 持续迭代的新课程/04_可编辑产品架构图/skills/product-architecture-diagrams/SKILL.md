...
- For a capability matrix, every cell must answer the intersection of its row and column. Do not use lifecycle column headers if the cell contents are only an unrelated module inventory.
- When one deliverable contains multiple views, keep capability names and product boundaries consistent across views; a module may move between visual groups, but its meaning must not silently change.
- HTML exports work: PNG and JPG buttons create downloads or a clear fallback message.
- After export, verify that the generated files are non-empty, have the intended dimensions, preserve Chinese labels, and contain only the diagram region rather than the toolbar.
